...
init(Args) ->
    ...
    SwarmWorkerOpts =
        [
         ...
         {libp2p_proxy,
          [{limit, application:get_env(blockchain_crawler, relay_limit, 25)}]},
         {libp2p_peerbook,
          [
           %{signed_metadata_fun, MetadataFun},
           {notify_peer_gossip_limit, application:get_env(blockchain_crawler, gossip_width, 100)},
           {notify_time, application:get_env(blockchain_crawler, peerbook_update_interval, timer:minutes(5))},
           {allow_rfc1918, application:get_env(blockchain_crawler, peerbook_allow_rfc1918, false)}
          ]},
         {libp2p_group_gossip,
          [
           ...
           {peerbook_connections, application:get_env(blockchain_crawler, outbound_gossip_connections, 2)},
           {inbound_connections, application:get_env(blockchain_crawler, max_inbound_connections, 6)},
           {peer_cache_timeout, application:get_env(blockchain_crawler, peer_cache_timeout, 10 * 1000)}
          ]}
        ] ++ GroupMgrArgs,
